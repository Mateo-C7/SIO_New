...
function cambiarIdioma() {
    $.i18n().load({
        en: './Scripts/languages/cliente_languages_en.json',
        es: './Scripts/languages/cliente_languages_es.json',
        br: './Scripts/languages/cliente_languages_br.json'
    }).done(function () {
        ReloadLanguage();
    });
...
function SaveLanguage(lang) {
    if (typeof sessionStorage !== 'undefined') {
        try {
            sessionStorage.setItem('clientelangFUP', lang);
        } catch (e) {
...
function getLanguage() {
    if (typeof sessionStorage !== 'undefined') {
        try {
            return sessionStorage.getItem('clientelangFUP');
        } catch (e) {
            return "es";
        }
    } else {
