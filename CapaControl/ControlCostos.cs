...
namespace CapaControl
{
    public class ControlCostos
    {
        ...
        public SqlDataReader poblarPlanta()
        {
            string sql = "";
            sql = "SELECT  planta_descripcion, planta_id, moneda.mon_descripcion + ' a ' + moneda_1.mon_descripcion AS Moneda  FROM            planta_forsa INNER JOIN " +
                        " moneda ON planta_forsa.planta_moneda_nal = moneda.mon_id INNER JOIN " +
                        " moneda AS moneda_1 ON planta_forsa.planta_moneda_ext = moneda_1.mon_id " +
                "WHERE  (planta_activo = 1) AND (planta_id IN (1, 2,11))";
            return BdDatos.ConsultarConDataReader(sql);
        }

        //Consulta las plantas por usuario
        public SqlDataReader ConsultaCorreoConfirmaCostos()
        {
            string sql = "";
            sql = "SELECT        empleado.emp_correo_electronico AS correo  FROM empleado INNER JOIN "+
                    " usuario ON empleado.emp_usu_num_id = usuario.usu_emp_usu_num_id WHERE(empleado.emp_estado_laboral = 'ACTIVO') "+
                    " AND (empleado.emp_correo_electronico IS NOT NULL) AND(usuario.usu_activo = 1) AND(usuario.usu_notifica_costoItem = 1)" +
                    " OR (empleado.emp_estado_laboral = 'ACTIVO') AND(empleado.emp_correo_electronico IS NOT NULL) AND " +
                    " (empleado.emp_mail_admin = 1) AND(usuario.usu_activo = 1) AND(usuario.usu_notifica_costoItem = 1)";
            return BdDatos.ConsultarConDataReader(sql);
        }

        //Consulta la moneda de trm planta
        public SqlDataReader ConsultarMonedaTrm(int planta)
        {
            string sql = "";
            sql = "SELECT 'TRM '+ moneda.mon_descripcion + ' a ' + moneda_1.mon_descripcion +':' AS Moneda  FROM            planta_forsa INNER JOIN " +
                        " moneda ON planta_forsa.planta_moneda_nal = moneda.mon_id INNER JOIN " +
                        " moneda AS moneda_1 ON planta_forsa.planta_moneda_ext = moneda_1.mon_id " +
                "WHERE  (planta_activo = 1) AND (planta_id ="+ planta + ")";
            return BdDatos.ConsultarConDataReader(sql);
        }

        //Consulta las plantas por usuario
        public SqlDataReader consultaTrimConfCostos(int planta, int anio)
        {
            string sql = "";
            sql = "SELECT Trimestre FROM Item_PrecioItem_Trim_Log "+
                    " WHERE(Activo = 1) AND(PlantaId ="+ planta + " ) AND(Anio = "+ anio + " ) AND(ConfirmaCosto = 1)";
            return BdDatos.ConsultarConDataReader(sql);
        }

        //Consulta la confirmacion de precios para el activar el boton
        public SqlDataReader consultaTrimConfPrecio(int planta, int anio, int trimestre)
        {
            string sql = "";
            sql = "SELECT isnull(ConfirmaPrecio,0 ) as ConfirmaPrecio   FROM Item_PrecioItem_Trim_Log " +
                    " WHERE(Activo = 1) AND(PlantaId =" + planta + " ) AND(Anio = " + anio + " ) AND(Trimestre = "+ trimestre + ")";
            return BdDatos.ConsultarConDataReader(sql);
        }


        //Consulta la trm del trimestre
        public SqlDataReader consultarTrimestre(int planta, int anio, int trimestre)
        {
            string sql = "";
            sql = "SELECT   ISNULL(Trm, 0) as Trm FROM Item_PrecioItem_Trim_Log "+
                  "  WHERE(Activo = 1) AND(PlantaId ="+planta+") AND(Anio = "+anio+") AND(Trimestre = "+trimestre+")";
            return BdDatos.ConsultarConDataReader(sql);
        }

        public String guardarConfirmaCosto(int planta, int anio, int trimestre,string usuario)
        {
            string sql;
            ...
            try
            {
                sql = "DECLARE @ultimoId INT; " + 
                      " INSERT INTO [dbo].[Item_PrecioItem_Trim_Log]" +
                      " ([PlantaId]  ,[Anio] ,[Trimestre]  ,[FechaCostos] ,[ConfirmaCosto],[Activo],[UsuConfirmaCosto]) " +
                      " values( "+planta+","+anio+","+trimestre + ", CONVERT(date, '" + fecha + "', 103) ," +1 + "," + 1 + ",'" + usuario+"'); " +
            ...
            {
                mensaje = "ERROR EN EL QUERY (INSERT EVENTO guardar confirma costo)";
            }
            return mensaje;
        }

        // eliminar costos del trimestre seleccionado
        public String EliminarCostoTrimestre(int planta, int anio, int trimestre, string usuario)
        {
            string sql;
            string fecha = System.DateTime.Now.ToString("dd/MM/yyyy");
            String mensaje = "";
            ...
            try
            {
                sql = "DELETE FROM  Item_Costo_Pv "+
                        " WHERE(Anio = "+@anio+") AND(Trimestre ="+ @trimestre+") AND(PlantaId ="+ @planta+"); ";
                consulta = BdDatos.CargarTabla(sql);
                foreach (DataRow row in consulta.Rows)
                {
                    mensaje = "1";
        ...
        // actualiza confirmacion de precios por Mercadeo
        public int guardarConfirmaPrecios(int planta, int anio, int trimestre, string usuario)
        {
            string sql;
            string fecha = System.DateTime.Now.ToString("dd/MM/yyyy");
            String mensaje = "";
            sql = "UPDATE Item_PrecioItem_Trim_Log set FechaPrecio = CONVERT(date, '" + fecha + "' ,103) , ConfirmaPrecio = 1, UsuConfirmaPrecio = '" + usuario+"' FROM Item_PrecioItem_Trim_Log " +
                    " WHERE(Activo = 1) AND(PlantaId = " + planta + ") AND(Anio = " + anio + ") AND(Trimestre =" + trimestre + ") ;";
            return BdDatos.Actualizar(sql);
            
        }

        public int guardarTrm(int planta, int anio, int trimestre, decimal trm)
        {
            string sql;
            string fecha = System.DateTime.Now.ToString("dd/MM/yyyy");
            String mensaje = "";
            sql = "UPDATE Item_PrecioItem_Trim_Log set Trm = "+ trm + " FROM Item_PrecioItem_Trim_Log "+
                    " WHERE(Activo = 1) AND(PlantaId = "+planta+") AND(Anio = "+anio+") AND(Trimestre ="+ trimestre+") ;";
            return BdDatos.Actualizar(sql);           
        }
        ...
        public int actualizarPlenoCop(int IdGrilla, int CodErp,  decimal plenoCop, decimal plenoUsd)
        {
            string sql;
            string fecha = System.DateTime.Now.ToString("dd/MM/yyyy");
            String mensaje = "";
            sql = "update Item_PrecioVentaItems set  PlenoCop = "+ plenoCop + " , PlenoUsd = " + plenoUsd +
                   " FROM Item_PrecioVentaItems " +
                   " WHERE(TempPrecios_Id = "+ IdGrilla + ") AND(CodErp = "+ CodErp+")";
            return BdDatos.Actualizar(sql);
        }

        //Consulta la confirmacion de costo del trimestre
        public SqlDataReader ConsultarConfirmaCosto(int planta, int anio, int trimestre)
        {
            string sql = "";
            sql = "SELECT  ISNULL(ConfirmaCosto, 0) AS ConfirmaCosto, FechaCostos, UsuConfirmaCosto "+
                   " FROM Item_PrecioItem_Trim_Log  "+
                   " WHERE(Activo = 1) AND(PlantaId ="+ @planta+") AND(Anio = "+@anio+") AND(Trimestre ="+ @trimestre+")";
            return BdDatos.ConsultarConDataReader(sql);
        }

        //Consulta el idItemPlanta
        public SqlDataReader ConsultarIdItemPlanta(int planta, int codigo)
        {
            string sql = "";
            sql = "SELECT   item_planta_id  FROM item_planta WHERE(cod_erp = "+ codigo + ") AND(planta_id = "+planta+") " ;
            return BdDatos.ConsultarConDataReader(sql);
        }


        public DataSet ConsultarCostosTrimestre(int Planta, int Anio, int Trimestre)
        {
            string sql;
            sql = " SELECT        planta_forsa.planta_pre AS Planta, Item_Costo_Pv.Anio, Item_Costo_Pv.Trimestre as Trim, " +
                  " Item_Costo_Pv.CodErp, Item_Costo_Pv.NombreItem, Item_Costo_Pv.Costo, Item_Costo_Pv.Observacion,  " +
                  " left (Item_Costo_Pv.Usuario,10) as  Usuario , Item_Costo_Pv.Fecha as Fecha  " +
                  " FROM            Item_Costo_Pv INNER JOIN  " +
                  " planta_forsa ON Item_Costo_Pv.PlantaId = planta_forsa.planta_id  " +
                  "  WHERE(Item_Costo_Pv.Anio = " + Anio + ") AND(Item_Costo_Pv.Trimestre = "+ Trimestre + ") AND(Item_Costo_Pv.PlantaId ="+ Planta + ") ";
            DataSet dsFUP = BdDatos.consultarConDataset(sql);
            // DataTable dt_temp = cambiarIdioma(ds_idioma.Tables[0]);
            return dsFUP;
        }

        public DataSet ConsultarPrecioCalculado(int Planta, int Anio, int Trimestre)
        {
            string sql;
            sql = " SELECT   Item_PrecioVentaItems.TempPrecios_Id AS Id, planta_forsa.planta_pre AS Planta, "+
                    " Item_PrecioVentaItems.Anio, Item_PrecioVentaItems.Trimestre AS Trim, Item_PrecioVentaItems.CodErp, " +
                    " Item_PrecioVentaItems.NombreItem, Item_PrecioVentaItems.Costo, Item_PrecioVentaItems.Trm, Item_PrecioVentaItems.PlenoCop, Item_PrecioVentaItems.PlenoUsd, Item_PrecioVentaItems.Fecha, " +
                    " left( Item_PrecioVentaItems.Usuario,10) as Usuario , Item_PrecioVentaItems.Observacion  FROM            Item_PrecioVentaItems INNER JOIN  " +
                    " planta_forsa ON Item_PrecioVentaItems.PlantaId = planta_forsa.planta_id " +
                    " WHERE (Item_PrecioVentaItems.Anio = "+ Anio + ") AND(Item_PrecioVentaItems.Trimestre = "+ Trimestre + ") AND(Item_PrecioVentaItems.PlantaId = "+ Planta + ") ";
            DataSet dsFUP = BdDatos.consultarConDataset(sql);
            // DataTable dt_temp = cambiarIdioma(ds_idioma.Tables[0]);
            return dsFUP;
        ...
        // Ejecuta procedimiento calculo precios mercadeo
        public int ProceCalculoPrecio(int planta, int anio, int trimestre,string usuario  )
        {
            ...
            sqls[0] = new SqlParameter("planta", planta);
            sqls[1] = new SqlParameter("anio", anio);
            sqls[2] = new SqlParameter("trimestre", trimestre);
            ...
            {
                // Creamos el Comando
                using (SqlCommand cmd = new SqlCommand("USP_CalculaPrecioItems", con))
                {
        ...
        // Ejecuta procedimiento inserta itemplanta precio USP_ActualizarPrecioItemsPv
        public int ProceInsertaPrecios(int planta, int anio, int trimestre, string usuario)
        {
            // Valor de retorno. Sabemos que si acaba valiendo -1 ha habido un error
            int Id = -1;
            string fecha = System.DateTime.Now.ToString("dd/MM/yyyy");
            ...
            {
                // Creamos el Comando
                using (SqlCommand cmd = new SqlCommand("USP_ActualizarPrecioItemsPv", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddRange(sqls);
        ...
        public int insertarCostoItems(int CodErp, string NombreItem, string Costo, string Observacion,int Anio,int Trimestre,
            int PlantaId,string Usuario)
        {
            // Valor de retorno. Sabemos que si acaba valiendo -1 ha habido un error
            int Id = -1;
            string fecha = System.DateTime.Now.ToString("dd/MM/yyyy");
            ...
            sqls[0] = new SqlParameter("CodErp", CodErp);
            sqls[1] = new SqlParameter("NombreItem", NombreItem);
            sqls[2] = new SqlParameter("Costo", Costo);
            sqls[3] = new SqlParameter("Observacion", Observacion);
            sqls[4] = new SqlParameter("Anio", Anio);
            sqls[5] = new SqlParameter("Trimestre", Trimestre);
            sqls[6] = new SqlParameter("PlantaId", PlantaId);
            ...
            {
                // Creamos el Comando
                using (SqlCommand cmd = new SqlCommand("USP_InsertaCostoItemPv", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddRange(sqls);
